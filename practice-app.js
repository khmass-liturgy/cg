(function(){
  ...
  function renderRepertoire(){
    const parts=['1st','2nd','3rd','파트 미지정','미지정'];
    const visibleMembers=activePart==='all'?memberOptions():memberOptions().filter(member=>member.part===activePart);
    const filtered=repertoire.filter(r=>(repFilter==='all'||r.stage===repFilter)&&(activePart==='all'||memberPart(r)===activePart)&&(activeMember==='all'||memberIdFor(r)===activeMember)).sort((a,b)=>(STAGES[a.stage]?.order||1)-(STAGES[b.stage]?.order||1));
    return `<div class="app-shell">
      ...
      <div class="unit-filter-wrap"><div class="unit-filter-label">파트별 보기</div><div class="filter-pills unit-pills">${[{id:'all',title:'전체'},...parts.map(part=>({id:part,title:part}))].map(part=>{const count=part.id==='all'?repertoire.length:repertoire.filter(r=>memberPart(r)===part.id).length;return `<button class="filter-pill${activePart===part.id?' active':''}" onclick="setPartFilter('${part.id}')">${html(part.title)} ${count}</button>`;}).join('')}</div><div class="unit-filter-label member-filter-label">담당 단원</div><div class="filter-pills unit-pills">${[{id:'all',name:'전체 단원'},...visibleMembers].map(member=>{const count=member.id==='all'?repertoire.filter(r=>activePart==='all'||memberPart(r)===activePart).length:repertoire.filter(r=>memberIdFor(r)===member.id).length;return `<button class="filter-pill${activeMember===member.id?' active':''}" onclick="setMemberFilter('${member.id}')">${html(member.name)} ${count}</button>`;}).join('')}</div><a class="member-source-link" href="https://khmass-liturgy.github.io/khcg/" target="_blank" rel="noopener">기타반 단원 관리 열기 ↗</a></div>
